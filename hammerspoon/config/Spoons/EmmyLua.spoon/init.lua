...
function M.processModule(module)
	if string.match(module.name, "hs") then
		io.write("---@meta " .. module.name .. "\n")
	end
...
function M.create(jsonDocs, prefix)
	...
	for _, module in ipairs(data) do
		...
		if fmtime == nil or mtime > fmtime then
			...
			io.close(fd)
			modules[#modules + 1] = module.name
		end
	end
	if prefix ~= "" or #modules == 0 then
		return
	end
	local fname = options.annotations .. "/init.lua"
	local fd = io.open(fname, "w+")
	assert(fd)
	io.output(fd)
	io.write([[
...
local hs
	...
	for _, modname in ipairs(modules) do
		io.write(string.format('%s = require("%s")\n', modname, modname))
	end
	io.close(fd)
end
